...
import com.realworld.dto.data.ArticleDataList;
import com.realworld.dto.data.ArticleFavoriteCount;
import com.realworld.entity.user.User;
import com.realworld.mapper.article.ArticleFavoriteReadService;
import com.realworld.mapper.article.ArticleReadService;
...
public class ArticleQueryService {
    ...
    public CursorPager<ArticleData> findUserFeedWithCursor(
        ...
        List<String> followUsers = userRelationshipQueryService.followedUsers(user.getId());
        if (followUsers.size() == 0) {
            return new CursorPager<>(new ArrayList<>(), page.getDirection(), false);
        } else {
            List<ArticleData> articles =
                    articleReadService.findArticlesOfAuthorsWithCursor(followUsers, page);
    ...
    public ArticleDataList findUserFeed(User user, Page page) {
        ...
        if (followUsers.size() == 0) {
            ...
            List<ArticleData> articles = articleReadService.findArticlesOfAuthors(followUsers, page);
            fillExtraInfo(articles, user);
            int count = articleReadService.countFeedSize(followUsers);
    ...
    private void setFavoriteCount(List<ArticleData> articles) {
        List<ArticleFavoriteCount> favoritesCounts =
                articleFavoriteReadService.articlesFavoriteCount(
    ...
    private void setIsFavorite(List<ArticleData> articles, User currentUser) {
        Set<String> favoritedArticles =
                articleFavoriteReadService.userFavorite(
